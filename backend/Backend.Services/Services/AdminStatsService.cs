    ...
    IRepository<Session> sessionRepository) : IAdminStatsService
{
    ...
    public async Task<double> GetSessionOccupancyAsync(int sessionId)
    {
        ...
        var sessionWithHall = await sessionRepository.GetFirstBySpecAsync(new SessionsByIdsSpec(new[] { sessionId }));
        int capacity = sessionWithHall?.Hall?.Capacity ?? 0;
    ...
    public async Task<List<TopMovieDto>> GetTopMoviesAsync()
    {
        ...
        return tickets
            .GroupBy(t => t.Booking.Session.Movie.MovieTitleUKR)
    ...
    public async Task<int> GetSpecialTicketsCountAsync(int? movieId, DateTime? from, DateTime? to)
    {
        ...
        return tickets.Count(t => t.Discount != null && t.Discount.Percentage > 0);
    }
    ...
    public async Task<List<SeatHeatmapDto>> GetHallHeatmapAsync(int hallId)
    {
        ...
        double averageSales = rawStats.Any() ? rawStats.Average(x => x.Count) : 0;

        var heatmap = rawStats.Select(seat => 
        {
            string color = seat.Count >= averageSales ? "Red" : "Blue";
