...
const generateDates = (daysCount: number = 10): DateOption[] => {
...
const DateSelector: React.FC<DateSelectorProps> = ({ activeDate, onDateSelect }) => {
  ...
  useEffect(() => {
    if (!activeDate && dates.length > 0) {
      onDateSelect(dates[0].formatted);
    }
  }, [activeDate, dates, onDateSelect]);
  ...
  const tomorrow = dates[1].formatted;
  const isCustomDate = activeDate !== today && activeDate !== tomorrow;
  ...
  return (
    <div className="flex justify-center gap-4 mb-20 flex-wrap relative z-20">
      ...
      <button
        ...
        className={getButtonClass(activeDate === today)}
      >
        Сьогодні {today}
      </button>
      ...
      <button
        ...
        className={getButtonClass(activeDate === tomorrow)}
      >
        Завтра {tomorrow}
      </button>
      ...
      <div className="relative" ref={dropdownRef}>
        ...
        >
          {isCustomDate ? `Обрано: ${activeDate}` : 'Обрати дату'} 
          <ChevronDown size={20} className={`transition-transform duration-300 ${isDropdownOpen ? 'rotate-180' : ''}`} />
        </button>

        {isDropdownOpen && (
          <div className="absolute top-full mt-2 right-0 w-64 bg-[#1e293b] border border-white/10 rounded-2xl shadow-xl overflow-hidden z-50">
            <div className="max-h-80 overflow-y-auto custom-scrollbar">
              {dates.map((date) => (
                <button
                  key={date.formatted}
                  onClick={() => handleSelect(date.formatted)}
                  className={`w-full text-left px-6 py-3 hover:bg-[#0753E0] hover:text-white transition-colors flex justify-between items-center ${
                    activeDate === date.formatted ? 'bg-[#0753E0] text-white' : 'text-white/80'
                  }`}
                >
                  <span className="capitalize">{date.dayName}</span>
                  <span className="font-mono">{date.formatted}</span>
                </button>
              ))}
            </div>
          </div>
