...
const MovieCard: React.FC<MovieCardProps> = ({ movie, isBlurred }) => {
  ...
  const hasActiveSessions = sessions.length > 0 && sessions[0] !== "Немає сеансів";

  const handleBookingClick = (e: React.MouseEvent, time: string) => {
    e.stopPropagation();
    goToBooking(e, time);
  };
          ...
          <div className="flex flex-wrap gap-2 justify-center">
            {hasActiveSessions ? (
              sessions.slice(0, 3).map((time: string) => (
                <button 
                  key={time} 
                  onClick={(e) => handleBookingClick(e, time)} 
