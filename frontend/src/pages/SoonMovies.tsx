...
const SoonMovies: React.FC = () => {
    ...
    return (
        <div className="relative w-full h-full font-['Inter']">
            ...
            <div
                className="absolute -top-50 right-0 w-150 h-150 bg-[#0753E0] rounded-full blur-[180px] opacity-30 pointer-events-none z-0"/>
            ...
            <div className="w-full max-w-360 mx-auto px-6 py-10 z-10 relative">
                ...
                {!loading && !error && movies.length > 0 && (
                    <div
                        className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-x-6 gap-y-12 place-items-center">
                        {movies.map((movie) => (
                            <MoviePreviewCard
                                key={movie.id}
                                movie={movie}
